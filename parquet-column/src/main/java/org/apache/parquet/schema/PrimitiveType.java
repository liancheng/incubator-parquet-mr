...
public final class PrimitiveType extends Type {
  ...
  protected Type union(Type toMerge, boolean strict) {
    ...
    if (PrimitiveTypeName.FIXED_LEN_BYTE_ARRAY == primitive) {
      if (strict && length != toMerge.asPrimitiveType().length) {
        throw new IncompatibleSchemaModificationException(
            "can not merge " + PrimitiveTypeName.FIXED_LEN_BYTE_ARRAY + " type " + toMerge + " into " + this);
      }
      builder.length(length);
    }

    if (getOriginalType() != null && getOriginalType() == toMerge.getOriginalType()) {
      if (getOriginalType() == OriginalType.DECIMAL) {
        if (!getDecimalMetadata().equals(toMerge.asPrimitiveType().getDecimalMetadata())) {
          throw new IncompatibleSchemaModificationException(
              "can not merge " + OriginalType.DECIMAL + " type " + toMerge + " into " + this);
        }
        builder.precision(getDecimalMetadata().getPrecision()).scale(getDecimalMetadata().getScale());
      }
      builder.as(getOriginalType());
    }
